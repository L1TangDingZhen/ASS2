...
#define SEQSPACE 12      /* #define SEQSPACE 7 to 12, the SEQSPACE needs to be double of windowsize */
...
/* 
1. remove the cumulative ACK from the windo, using a booleam array acked to track which packets have been ACKed.
2. wjem receive a new ACK, mark the order as true in the acked array.
3. only sliding the window when the first packet in the window has been ACKed.
4. if window is not empty, start the timer for the first unacked packet.
*/
